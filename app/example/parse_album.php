...
class parse_album extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:parse_album';
    ...
    public function handle()
    {
        ...
        $handle = fopen("storage/ParseArray", "r"); // открываем файл на чтение
        if ($handle) { 
            while (($line = fgets($handle)) !== false) { // цикл пока попытка взять след. строку не закончится неудачей
                ...
                if ($html->size > 0){
                    ...
                    //$this->info('$box_download_link = '.$box_download_link);
                    
                    if ($html->find('div.story h1', 0) != NULL) {
                    ...
                    if ($html->find('span[itemprop="author"]', 0) != NULL) {
                        ...
                        //$album_isset = Album::where('title', $albumName) -> where('artist', $artist) -> firsf();
                        ...
                        )
                    {
                        $timeAndSizeFinder = explode('<b>',$html->find('div[itemscope]',0)-> outertext);
                            // $finalTime = explode('</b>: ',$timeAndSizeFinder[7])[1];                    
                            // $finalSize = explode('</b>: ',$timeAndSizeFinder[8])[1];
                        ...
                        $array_to_db = [
                            ...
                            'tracklist' => $trackList,
                            'description' => $html->find('div.quote',0)-> innertext,
                        ...
                        if ($point_error == false){
                            ...
                            file_put_contents('storage/ErrorAlbumsList', $line.PHP_EOL, FILE_APPEND | LOCK_EX);
                        }   
                    }
